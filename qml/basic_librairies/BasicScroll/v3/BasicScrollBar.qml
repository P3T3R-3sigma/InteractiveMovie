...
BasicItem {
    ...
    id: mTHIS_BASICSCROLLBAR_V2

    /**
        The elements of the scroll are BasicItem, we add them with the function 'addNewElements'

        The elements (BasicItem) should have mTHIS_BASICSCROLLBAR_V2 as parent
    ...
    /*
        Variables
      */
    property var mListBasicItemEmptySlot: []
    property var mListBasicItemInScroll: []
    ...
    Component {
        id: mTHIS_COMPONENT_EMPTY_SLOTS

        BasicImageSource {
            id: mTHIS_BASICIMAGESOURCE

            property real mSlotRatioHeightToWidth: getmSourceSizeHeight() / getSourceSizeWidth()

            onMSlotRatioHeightToWidthChanged: {
                if (mTHIS_BASICSCROLLBAR_V2.mSLOT_RATIO_HEIGHT_TO_WIDTH <= 0 && mSlotRatioHeightToWidth) {
                    mTHIS_BASICSCROLLBAR_V2.mSLOT_RATIO_HEIGHT_TO_WIDTH = mSlotRatioHeightToWidth;
                    mTHIS_BASICSCROLLBAR_V2.drawScroll();
                }
            }
    ...
    function addNewBasicItemToScrollBar(sBasicItem, sIdx) {
        if (sBasicItem.parent !== mTHIS_BASICSCROLLBAR_V2) {
            mBasicDisplay.error(mTHIS_BASICSCROLLBAR_V2, "The BasicItem's parent should be the scroll bar"
                                + "; Actual parent= " + mBasicDisplay.getName(sBasicItem.parent)
                                + "; Expected parent= " + mBasicDisplay.getName(mTHIS_BASICSCROLLBAR_V2))
        }
        if (mBasicNum.isNull(sIdx)) {
            mListBasicItemInScroll.push(sBasicItem);
        } else {
            mListBasicItemInScroll.splice(sIdx, 0, sBasicItem);
        }
        drawScroll();
    }
    ...
    function withdrawElement(sIdxOrBasicItem) {
        ...
        if (typeof sIdxOrBasicItem === "number") {
            ...
            lIdx = mListBasicItemInScroll.indexOf(sIdxOrBasicItem);
            if (lIdx < 0) {
                mBasicDisplay.error(mTHIS_BASICSCROLLBAR_V2, "The BasicItem is not in the list"
        ...
        if (lIdx < 0 || lIdx >= mListBasicItemInScroll.length) {
            ...
            mListBasicItemInScroll.splice(lIdx, 1);
            drawScroll();
        }
    }
    ...
    function createEmptySlots() {
        if (mNUMBER_SLOTS > 0 && mSOURCE_SIMPLE_EMPTY_SLOTS != "" && mNUMBER_SLOTS != mListBasicItemEmptySlot.length
            ...
            while (mListBasicItemEmptySlot.length < mNUMBER_SLOTS) {
                let lBasicImage = mTHIS_COMPONENT_EMPTY_SLOTS.createObject(mTHIS_BASICSCROLLBAR_V2);
                /*
                  Initiate
                  */
    ...
    function drawScroll() {
        mBasicDisplay.checkParentHasDimension(mTHIS_BASICSCROLLBAR_V2);
        /*
          Draw empty slots
          */
    ...
    function getmListItemToCheckIfLoaded() {
        ...
        /*
          Arrows
          */
        lListToCheck.push(getmWindowArrowLeft())
        lListToCheck.push(getmWindowArrowRight())
        /*
          Empty slots
          */
        if (mListBasicItemEmptySlot && mListBasicItemEmptySlot.length > 0) {
            lListToCheck = lListToCheck.concat(mListBasicItemEmptySlot)
        }
        /*
          Pictos
          */
        if (mListBasicItemInScroll && mListBasicItemInScroll.length > 0) {
            lListToCheck = lListToCheck.concat(mListBasicItemInScroll)
        }
        /*
          Return
          */

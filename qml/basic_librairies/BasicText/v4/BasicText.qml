...
BasicItem {
    ...
    property real radiusPctHeight: 0.02
    property real radiusInteriorPctHeight: 0.02
    property color borderColor: Qt.color("transparent")
    property real borderWidthPercent: 0
    property real paddingPercentWidth: 0.0 //0.05
    property real paddingPercentHeight: 0.0 //0.15
    /*
      Text itself
      */
    ...
    property real textFontForceSizePixel: -1        // carefull, can lead to the text going outside of the window
    property real textFontForceSizePixelMax: -1
    property real textFontPixelSizePercentMax     // imposes a maximum to the font size (useful if fit to window, but want to limit)
    /*
      Options
      */
    ...
    Rectangle {
        ...
        /*
          ...
          */
        color: borderColor
        radius: parentHeight * radiusPctHeight
        opacity: backgroundOpacity
        width: parent.width
        height: parent.height

        ////////////////////////////////////////////
        BasicDebugRectangle {visible: mTHIS_BASICTEXT_V4.mIS_DEBUG}
        ////////////////////////////////////////////
        ...
        Rectangle {
          ...
          Default values
          */
            color: mTHIS_BASICTEXT_V4.backgroundColor
            radius: mTHIS_BASICTEXT_V4.parentHeight * mTHIS_BASICTEXT_V4.radiusInteriorPctHeight
            // width: mBorder.width - 2 * Math.ceil(mTHIS_BASICTEXT_V4.borderWidthPercent * mTHIS_BASICTEXT_V4.parentWidth)
            // height: mBorder.height - 2 * Math.ceil(mTHIS_BASICTEXT_V4.borderWidthPercent * mTHIS_BASICTEXT_V4.parentWidth)
            opacity: backgroundOpacity
            ...
            AppText {
                ...
                z: mTHIS_BASICTEXT_V4.z

                visible: mTHIS_BASICTEXT_V4.visible

                /*
                  Text
                  */
                text: mTHIS_BASICTEXT_V4.text
                color: textColor
                opacity: textOpacity
                ...
                font.pixelSize: {
                    if (textFontForceSizePixel > 0) {
                        ...
                        if (textFontPixelSizePercentMax > 0) {
                            return Math.ceil(Math.min(textFontPixelSizePercentMax, textFontPixelSizePercent) * mTHIS_BASICTEXT_V4.parentHeight);
                        } else {
                            return Math.ceil(textFontPixelSizePercent * mTHIS_BASICTEXT_V4.parentHeight);
                        }
                    }
                }
    ...
    function startBumpText() {
        mThisNumberAnimationTextBump.to = Math.ceil(textFontPixelSizePercent * mTHIS_BASICTEXT_V4.parentHeight)
    ...
    function startBumpFullWindow() {
        ...
        mThisSubAnimeText.from = Math.ceil(textFontPixelSizePercent * 0.1 * mTHIS_BASICTEXT_V4.parentHeight);
        mThisSubAnimeText.to = Math.ceil(textFontPixelSizePercent * mTHIS_BASICTEXT_V4.parentHeight);
    ...
    function getmBoundingRectHeight() {
        if (mBasicNum.isNull(parent) || parentHeight == 0) {
            return 0
        }
        return mAppText.contentHeight / parentHeight;
    }
    function getmBoundingRectWidth() {
        if (mBasicNum.isNull(parent) || parentWidth == 0) {
            return 0
        }
        return mAppText.contentWidth / parentWidth;
    }
    ...
    function shadow_getmTHIS_BASICTEXT_V4WindowText_width() {
        return mTHIS_BASICTEXT_V4WindowText.width;
    }
    function shadow_getmTHIS_BASICTEXT_V4WindowText_height() {
        return mTHIS_BASICTEXT_V4WindowText.height;
    }
    function shadow_getmTHIS_BASICTEXT_V4WindowText() {
        return mTHIS_BASICTEXT_V4WindowText
    }
